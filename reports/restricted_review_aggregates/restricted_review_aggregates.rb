...
CSV.open(filename, 'wb',
  ...
  repos.each do |repo|
    ...
    resource_records.each do |resource_record|
      ...
      records_by_uri = batch_get_records_by_uris(ao_uris)
      records_by_uri.each do |ao_uri, ao_record|
        ...
        accessrestrict_review = ao_record['notes'].select do |note|
          note['type'] == 'accessrestrict' && note.dig('rights_restriction', 'local_access_restriction_type', 0) == 'Review'
        end
