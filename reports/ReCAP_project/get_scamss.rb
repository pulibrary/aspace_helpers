...
CSV.open("mss_linear_feet.csv", "a",
  ...
  resources.each do |resource|
    ...
    row << [
      ...
      (linear_feet[0]['number'] unless linear_feet.empty?).to_s,
      ("#{structured_container_count[0]['number']} #{structured_container_count[0]['extent_type']}" unless structured_container_count.empty?).to_s,
      (linear_feet[0]['container_summary'] unless linear_feet.empty?).to_s
    ]
  end
end
