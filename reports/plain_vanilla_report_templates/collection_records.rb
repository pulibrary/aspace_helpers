...
CSV.open(output_file, "a",
  ...
  resource_ids.each do |resource_id|
    ...
    ao_refs = ao_tree['uris']
    #toggle collection-level line on or off
    #ao_refs = ao_refs.reject { |ao_ref| ao_ref.dig('level') == 'collection' }

    #fetch all records in batches of 250, then top containers once each via cache
    records_by_uri = batch_get_records_by_uris(ao_refs.map { |ao_ref| ao_ref['ref'] })

    ao_refs.each do |ao_ref|
      @depth = ao_ref['depth']
      uri = ao_ref['ref']
      get_ao = records_by_uri[uri]
        ...
        get_ao.dig('instances').each do |instance|
          ...
          top_container_record =
            if instance.dig('sub_container').nil? == false
              get_record_cached(instance['sub_container']['top_container']['ref'])
            else
              if instance.dig('top_container')
                get_record_cached(instance['top_container']['ref'])
              end
            end
          top_container =
